...
import { Home, FileText, Settings, Plus, BookOpen, X, ChevronRight, ArrowLeft } from "lucide-react";
    ...
    <>
      <div className="fixed bottom-0 left-0 right-0 z-[100]">
        <nav className="relative z-10">
          <div className="mx-auto max-w-2xl px-3 pt-2 pb-safe">
      ...
      {showCreatePopup && (
        <div className="fixed inset-0 z-[200] animate-fadeIn">
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-end justify-center max-h-[90vh]">
            <div className="w-full max-w-2xl mx-auto bg-white rounded-t-[32px] overflow-hidden animate-slideUp">
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-bold text-gray-900">Create New Paper</h2>
                  <button onClick={closePopup} className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center active:bg-gray-200 transition-colors">
                  ...
                  <div className="space-y-3">
                    ...
                    {CLASSES.map((classInfo) => (
                      <button
                        ...
                        className="w-full flex items-center gap-3 bg-gray-50 rounded-xl p-4 active:bg-gray-100 transition-colors min-h-[64px]"
                      >
                ...
                {showSubjectSelector && (
                  <div className="space-y-3">
                    <button onClick={() => setShowSubjectSelector(false)} className="text-sm text-[#1E88E5] font-medium mb-4 flex items-center gap-2 min-h-[48px] px-2">
                      ...
                      {SUBJECTS.map((subject, index) => {
                        ...
                        return (
                          <button
                            ...
                            className="w-full flex items-center gap-3 bg-gray-50 rounded-xl p-4 active:bg-gray-100 transition-colors min-h-[56px]"
                          >
                            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${colorClass} flex items-center justify-center shadow-md`}>
                              <BookOpen className="w-5 h-5 text-white" />
                            </div>
                            <span className="font-medium text-gray-900 flex-1 text-left">{subject.name}</span>
                            <ChevronRight className="w-5 h-5 text-gray-400" />
                          </button>
                        );
                      })}
